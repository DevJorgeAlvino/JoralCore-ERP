...
$highlightClasses = $plugin->getHighlightQueryClasses() ?? 'text-primary-500 font-semibold hover:underline';
$highlightStyles = $plugin->getHighlightQueryStyles() ?? '';
@endphp
                ...
                @foreach ($result->details as $label => $value)
                    ...
                    >
                        ...
                        @php
                            ...
                            $highlightedValue = $plugin->isMustHighlightQueryMatches()
                                ? \CharrafiMed\GlobalSearchModal\Utils\Highlighter::make($escapedValue, $this->search, $highlightStyles, $highlightClasses)
                                : $escapedValue;
                        @endphp
                        <dd class="inline">{!! $highlightedValue !!}</dd>
                    </div>
                @endforeach
            </dl>
        @endif
